...
async function fetchMatches() {
  ...
  const mentor = await db.collection('matches').where('mentorId', '==', decoded.uid).get();
  const mentee = await db.collection('matches').where('menteeId', '==', decoded.uid).get();
...
export async function MatchesList() {
  ...
  if (matches.length === 0) {
    return <p className="mt-6 text-sm text-slate-500">No active mentorships yet.</p>;
  }
